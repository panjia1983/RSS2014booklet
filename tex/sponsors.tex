\chapter{Sponsors}
\thispagestyle{empty} %For the back page full of sponsors

\newcommand{\placePlatiumSponsorLogo}[1]{
     \raisebox{-0.5\height}{\includegraphics[width=6cm,height=2.7cm, keepaspectratio]{local_img/sponsors/#1}}
}

\newcommand{\placeGoldSponsorLogo}[1]{
     \raisebox{-0.5\height}{\includegraphics[width=5cm,height=2.7cm, keepaspectratio]{local_img/sponsors/#1}}
}

\newcommand{\placeSilverSponsorLogo}[1]{
     \raisebox{-0.5\height}{\includegraphics[width=7cm,height=2.7cm, keepaspectratio]{local_img/sponsors/#1}}
}

\newcommand{\placeBronzeSponsorLogo}[1]{
     \raisebox{-0.5\height}{\includegraphics[width=5.0cm,height=1.8cm, keepaspectratio]{local_img/sponsors/#1}}
}

\newcommand{\placeSupporterLogo}[1]{
     \raisebox{-0.5\height}{\includegraphics[width=5cm,height=1.5cm, keepaspectratio]{local_img/sponsors/#1}}
}

\def \separationwidth {1cm}

\vspace{-3cm} %no header
\noindent
The conference gratefully acknowledges the following sponsors.  In
addition to other benefits, these sponsors allow the conference to
keep registration and workshop fees to a minimum, particularly for
students.\\

\begin{center}
%\vspace{-8mm}
%\vspace{5mm}
\rule{12cm}{1pt}\\[12pt]


\textsf{\textbf{\Large PLATINUM SPONSORS}}\\[12pt]

\placePlatiumSponsorLogo{Anki}
\hspace{\separationwidth}
\placeSilverSponsorLogo{NEW_Glogo_flat_CMYK_Coated}

\vspace{5mm}
\rule{12cm}{1pt}\\[12pt]
\textsf{\textbf{\Large GOLD SPONSORS}}\\[5mm]
\placeGoldSponsorLogo{ABB}

\vspace{5mm}
\rule{12cm}{1pt}\\[12pt]
\textsf{\textbf{\LARGE SILVER SPONSORS}}\\[5mm]

\placeSilverSponsorLogo{MSFT_logo_c_C-Gray}
\hspace{\separationwidth}
\placeSilverSponsorLogo{Bosch}
\\
\vspace{\separationwidth}
\placeSilverSponsorLogo{Rethink}
\hspace{\separationwidth}
\placeSilverSponsorLogo{NASA-JPL}
\\
\vspace{\separationwidth}
\placeSilverSponsorLogo{Nvidia}
\hspace{\separationwidth}
\placeSilverSponsorLogo{LockheedMartin}
\hspace{\separationwidth}

\vspace{5mm}
\rule{12cm}{1pt}\\[12pt]

\clearpage
\rule{12cm}{1pt}\\[12pt]
\textsf{\textbf{\Large BRONZE SPONSORS}}\\[5mm]
\placeBronzeSponsorLogo{Disney}
\hspace{\separationwidth}
\placeBronzeSponsorLogo{BostonDynamics}
\hspace{\separationwidth}
\placeBronzeSponsorLogo{Savioke}
\\
\vspace{\separationwidth}
\placeBronzeSponsorLogo{IntuitiveSurgical}
\hspace{\separationwidth}
\placeBronzeSponsorLogo{Phasespace}

\vspace{5mm}
\rule{12cm}{1pt}\\[12pt]
\textsf{\textbf{\Large This conference is supported by}}\\
\vspace{2mm}

\placeSupporterLogo{Barrett}
\hspace{\separationwidth}
\placeSupporterLogo{Springer_cmyk}
\hspace{\separationwidth}
\placeSupporterLogo{arl}



\vspace{7mm}
\textsf{\textbf{\Large Best Paper Award sponsored by}}\\
\vspace{3mm}
\includegraphics[height=1cm]{local_img/sponsors/Springer_cmyk}
\\

%\vspace{1mm}
\rule{12cm}{1pt}\\[10pt]
\textsf{\textbf{\Large Organized by }}
\hspace{2mm}
\raisebox{-0.3\height}{\includegraphics[height=1.5cm]{local_img/berkeley}}
%\includegraphics[height=2cm]{local_img/TU_Logo_lang_4c_rot}

\vspace{5mm}
\rule{12cm}{1pt}\\[10pt]
\end{center}
%{\scriptsize Intel and the Intel logo are trademarks of Intel Corporation in the Unites States and other countries. }
